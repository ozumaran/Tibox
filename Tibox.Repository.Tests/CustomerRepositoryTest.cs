...
namespace Tibox.DataAccess.Tests
{
    ...
    public class CustomerRepositoryTest
    {
        ...
        {
            var customer = _unitOfWork.Customers.GetEntityById(95);
        ...
        [TestMethod]
        public void OrderByOrderNumber()
        {
            var orden = _unitOfWork.Orders.OrderByOrderNumber("542382");
            Assert.AreEqual(orden != null, true);
            Assert.AreEqual(orden.Id>0, true);
        }

        [TestMethod]
        public void OrderWithOrderItems()
        {
            var orden = _unitOfWork.Orders.OrderWithOrderItems("542382");
            Assert.AreEqual(orden != null, true);
            Assert.AreEqual(orden.OrderItems.Any(), true);
        }
    }
}
